...
{
    static uint8_t first_power_on_flag = 1;
    xSemaphoreTake(WakeupMutex, portMAX_DELAY);
    ...
    if(first_power_on_flag != 1)
    {
        ...
        int groupId=0;
        groupId=get_light_wake_group();
        if(7 == groupId)
        {
            groupId=0;
        }
        cmd_info_change_cur_model_group(groupId);
        ignore_asr_msg++;
...
{
    int ret=0;
    ret=get_light_wake_group();
    if((ret>=1)&&(ret<=6))//命令词网络0和默认灯网络不进入睡眠
    {
        (void)xTimer;
        sys_msg_t send_msg;
        send_msg.msg_type = SYS_MSG_TYPE_CMD_INFO;
...
void sys_deal_asr_msg(sys_msg_asr_data_t *asr_msg)
{
    ...
    if(MSG_ASR_STATUS_GOOD_RESULT == asr_msg->asr_status && (ignore_asr_msg == 0))
    {
        ...
        {
            /*updata wakeup state*/
            uint32_t wakeup_word_id = cmd_info_get_semantic_id(cmd_handle);
            if (SYS_STATE_WAKEUP == get_wakeup_state())
            {
                light_blink_output();//如果唤醒状态也要闪一下
            }
            else
            {
                if(get_light_wake_group() == wakeup_word_id)
                {
                    light_blink_output();
                    enter_wakeup_deal(EXIT_WAKEUP_TIME,cmd_handle); 
                }
            }
        ...
        else if (SYS_STATE_WAKEUP == get_wakeup_state())
        {
            ...
            ci_loginfo(LOG_USER,"asr cmd_id:%d,semantic_id:%d\n", cmd_id, semantic_id);
            ...
            #else
            userapp_deal_asr_msg_by_semantic_id(semantic_id);
            #endif
        }
