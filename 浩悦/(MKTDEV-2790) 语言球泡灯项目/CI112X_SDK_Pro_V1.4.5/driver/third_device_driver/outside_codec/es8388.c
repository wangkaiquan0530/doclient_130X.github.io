 ...
 * @return int32_t 正确
 */
// TODO: 8388 init 播放要用，录音要用，又不知道录音先初始化还是播放先初始化，但是这套初始化有录音有放音
int32_t ES8388_play_init(es8388_num_sel_t num_8388)
{
    es8388_alc_config_t alc_config_str;
    memset(&alc_config_str,0,sizeof(es8388_alc_config_t));
    es8388_input_cha_t input_cha_str = {0};
    memset(&input_cha_str,0,sizeof(es8388_input_cha_t));
    uint32_t regnum = 0;
