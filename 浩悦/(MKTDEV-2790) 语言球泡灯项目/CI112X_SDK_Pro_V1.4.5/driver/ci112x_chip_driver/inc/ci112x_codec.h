/**
 * @file ci112x_codec.h
 * @brief CI112X inner codec驱动的头文件
 * @version 0.1
 * @date 2019-05-10
 * 
 ...
 * @defgroup ci112x_codec ci112x_codec
 * @brief CI112X芯片inner CODEC驱动
 * @{
 */
...
typedef enum
{
    ...
    //!MIC增益为12dB
    INNER_CODEC_MIC_AMP_12dB = 2,
    //!MIC增益为20dB
    INNER_CODEC_MIC_AMP_20dB = 3,
...
typedef enum
{
    ...
    INNER_CODEC_NOISE_GATE_THRE_63dB = 4, 
    INNER_CODEC_NOISE_GATE_THRE_69dB = 5, 
    INNER_CODEC_NOISE_GATE_THRE_75dB = 6,
    INNER_CODEC_NOISE_GATE_THRE_81dB = 7,  
...
{
    //!MIC L 增益设置
    inner_codec_mic_amplify_t  codec_adcl_mic_amp;
...
typedef struct 
{
    ...
    //!使用CI112X的ALC控制inner CODEC还是使用其本身的ALC控制
    inner_cedoc_gate_t use_ci112x_alc;
    //!ALC开关
...
void inner_codec_alc_right_config(inner_codec_alc_use_config_t* ALC_str);
void inner_codec_adc_dig_gain_set_left(uint8_t gain);
void inner_codec_adc_dig_gain_set_right(uint8_t gain);
void inner_codec_set_mic_gain_left(inner_codec_mic_amplify_t gain);
void inner_codec_set_mic_gain_right(inner_codec_mic_amplify_t gain);
uint8_t inner_codec_get_alc_gain_left(void);
uint8_t inner_codec_get_alc_gain_right(void);
...
void inner_codec_right_mic_reinit(void);
