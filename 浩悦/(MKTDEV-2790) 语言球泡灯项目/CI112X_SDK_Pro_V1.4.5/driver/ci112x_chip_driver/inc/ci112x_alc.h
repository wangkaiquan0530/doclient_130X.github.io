...
#ifndef __CI112X_ALC_H
#define __CI112X_ALC_H
...
typedef struct 
{
    ...
    //ALC输出的增益配置，需要送往CODEC的数字增益配置寄存器地址
    uint32_t      cfg_addr;
...
typedef enum
{
    //ALC左通道的输入为inner CODEC左通道输出
    ALC_L_GAIN_FROM_CODEC_L = 0,
    //ALC左通道的输入为inner CODEC右通道输出
    ALC_L_GAIN_FROM_CODEC_R = 1,
...
typedef enum
{
    //ALC右通道的输入为inner CODEC左通道输出
    ALC_R_GAIN_FROM_CODEC_R = 0,
    //ALC右通道的输入为inner CODEC右通道输出
    ALC_R_GAIN_FROM_CODEC_L = 1,
