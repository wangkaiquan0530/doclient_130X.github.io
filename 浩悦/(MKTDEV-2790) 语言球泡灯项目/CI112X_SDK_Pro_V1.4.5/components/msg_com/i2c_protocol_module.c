...
static uint32_t i2c_port_send_packet_rev_msg(sys_msg_i2c_data_t *msg)
{
    ...
    memcpy((uint8_t *)(&send_msg.msg_data), msg, sizeof(sys_msg_i2c_data_t));
    ...
    switch(i2c_recv_status)
    {
        ...
        {          
             if(data == IIC_END_DATA)
             {
                ...
                #if IIC_PROTOCOL_SEC_ID
                memset(send_secid_packet, 0, sizeof(send_secid_packet));
                #endif
                memset(send_cmdid_packet, 0, sizeof(send_cmdid_packet));
                
                recv_cmdid_packet.end = data;
                sys_msg_i2c_data_t i2c_msg;
                memcpy((uint8_t *)(&i2c_msg.cmdid_msg), &recv_cmdid_packet, sizeof(i2c_cmdid_msg_t));
...
static uint32_t i2c_deal_cmdid_msg(sys_msg_i2c_data_t *i2c_msg)
{
    ...
    memcpy((uint8_t *)(&msg), &(i2c_msg->cmdid_msg), sizeof(i2c_cmdid_msg_t));
    ...
    switch(i2c_recv_status)
    {
        ...
        {            
             if(data == IIC_END_DATA)
             {
                ...
                sys_msg_i2c_data_t i2c_msg;
                memcpy((uint8_t *)(&i2c_msg.secid_msg), &recv_secid_packet, sizeof(i2c_secid_msg_t));
                i2c_port_send_packet_rev_msg(&i2c_msg);
             }
             else
             {
...
static uint32_t i2c_deal_secid_msg(sys_msg_i2c_data_t *i2c_msg)
{
    ...
    memcpy((uint8_t *)(&msg), &(i2c_msg->secid_msg), sizeof(i2c_secid_msg_t));
...
uint32_t i2c_send_data(volatile uint8_t *reg)
{
    switch(*reg){
        ...
        #if IIC_PROTOCOL_CMD_ID
        case I2C_REG_CMDID:
            IIC0->TXDR = send_cmdid_packet[send_count++];
        ...
        #if IIC_PROTOCOL_SEC_ID
        case I2C_REG_SECID:
        	IIC0->TXDR = send_secid_packet[send_count++];
...
{
    #if IIC_PROTOCOL_CMD_ID
    memcpy((uint8_t *)(send_cmdid_packet), cmdid_msg_t, sizeof(send_cmdid_packet));
    #endif
    
    #if IIC_PROTOCOL_SEC_ID
    memcpy((uint8_t *)(send_secid_packet), secid_msg_t, sizeof(send_secid_packet));
    #endif
}
...
void i2c_communicate_init(void)
{
    ...
    i2c_io_init();
    /*IIC初始化为SLAVE模式*/
    i2c_slave_init(IIC0,I2C_PROTOCOL_SPEED,LONG_TIME_OUT);
}
