...
void send_ack_get_info_packet(void)
{
    ...
    memcpy(&send_data_buf[8],(char*)&FileConfigInfo,FileConfigInfoLen);
...
{
    sprintf((char*)send_data_buf, "%d:%d/%d", index,current,total);
    make_packet(MSG_TYPE_NOTIFY, MSG_CMD_UPDATE_PROGRESS, 0, send_data_buf, strlen((char*)send_data_buf)+1);
    send_func();
}
