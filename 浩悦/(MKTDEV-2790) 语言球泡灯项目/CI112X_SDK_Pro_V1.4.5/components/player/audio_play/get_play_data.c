...
{
    memset(&data_list,0x0,sizeof(get_play_data_msg_t));
    memcpy(&data_list,msg,sizeof(get_play_data_msg_t));
...
int32_t check_ci_voice_head(ci_voice_head_t *wave_head, uint32_t *wave_lens, uint32_t *head_lens,prompt_decoder_config *config,uint32_t *threshold_data_size)
{
    ...
    *threshold_data_size = 0;
    ...
    if(0 == strncmp(wave_head->wave_ci_mp3.ID3,"ID3",3))
    {
        if(0 == strncmp(wave_head->wave_ci_mp3.CI,"CI",2))
        {
    ...
    if(0 == strncmp(wave_head->wave_ci_flac.FLAC,"fLaC",4))
    {
        if(0 == strncmp(wave_head->wave_ci_flac.vorbis_comment.ci_flag_str,"filesize=",9))
        {
    ...
    #endif
    {
        if(wave_head->wave_pcm_44.wfmt.wav_formatag == 0x0011)
        {
            ...
            if (0 == strncmp(wave_head->wave_pcm_48.dchunk.data, "data", 4))
            {
            ...
            else if(0 == strncmp(wave_head->wave_adpcm.dchunk.data, "data", 4))
            {
        ...
        else
        {
            ...
            switch (fmt_chunk_size)
            {
                case 0x10:
                {
                    ...
                    if (0 == strncmp(wave_head->wave_pcm_44.dchunk.data, "data", 4))
                    {
    ...
    //*threshold_data_size = 0;  //用于关闭“提前开ASR功能”
    //#endif
...
int32_t process_flash_addr_list(char *data_buf, uint32_t size, uint32_t flash_addr, uint32_t *bytesLeft, uint32_t *threshold_data_size)
{
    ...
    post_read_flash(data_buf, flash_addr, size);
    ...
    #endif
    memset(data_buf,0x0,wave_head_size);
    *data_buf = 's';
    *(data_buf+1) = 'p';
    *(data_buf+2) = 'l';
    *(data_buf+3) = 'c';
    ...
    else
    {
        switch(config.mode)
        {
            ...
            {
                memset(data_buf+wave_total_lens+wave_head_size,0x0,size-wave_total_lens-wave_head_size);
                break;
            }
        }
        *bytesLeft = 0;
    ...
    for(;;)
    {
        switch(get_data_state)
        {
            ...
            case GET_PLAY_DATA_STATE_START:
            {
                ...
                {
                    memcpy(&msg,&data_list,sizeof(get_play_data_msg_t));
                ...
                switch(msg.type)
                {
                    case GET_PLAY_DATA_QSPI_FLASH :
                    {
                        ...
                        if(msg.spi_flash_msg.data_addr == wakeup_voice_addr)
                        {
                            ...
                            set_play_will_finish_threshold_data_size(wakeup_voice_threshold_data_size);
                            #if AUDIO_PLAY_SUPPT_IMAADPCM_PROMPT
                        ...
                        #endif
                        {
                            ...
                            { 
                                switch(config.mode)
                                {
                                    ...
                                    case CI_ADPCM_DECODER_MODE_FLAC:
                                    {
                                        memset(data_buf+wave_total_lens+wave_head_size,0x0,GET_PLAY_DATA_BUFF_SIZE-wave_total_lens-wave_head_size);
                                        break;
                                    }
                                }
                    ...
                    case GET_PLAY_DATA_QSPI_FLASH_LIST:
                    {
                        ...
                        do{
                            ...
                            {
                                memset((char *)(data_buf+flash_read_size),0x0,GET_PLAY_DATA_BUFF_SIZE-flash_read_size);
            ...
            case GET_PLAY_DATA_STATE_BUSY:
            {
                ...
                if(msg.type != GET_PLAY_DATA_NET)
                {
                    ...
                    if(data_size < GET_PLAY_DATA_BUFF_SIZE)
                    {
                        /* 文件结束最后一笔数据 */
                        read_size = data_size;
                        memset((void *)(data_buf+buf_index*GET_PLAY_DATA_BUFF_SIZE),0x00,GET_PLAY_DATA_BUFF_SIZE);
                        get_data_state = GET_PLAY_DATA_STATE_STOP;
                ...
                switch(msg.type)
                {
                    case GET_PLAY_DATA_QSPI_FLASH :
                    {
                        ...
                        {
                            //如果该命令词地址已常驻内存，直接从内存拷贝
                            memcpy((char *)(data_buf+buf_index*GET_PLAY_DATA_BUFF_SIZE),(char *)(wakeup_voice_buf+GET_PLAY_DATA_BUFF_SIZE*first_flag),read_size);
                        }
                        else
                        #endif
                        {
                    ...
                    case GET_PLAY_DATA_QSPI_FLASH_LIST:
                    {
                        ...
                        if((msg_data_flag == AUDIO_DATD_BUFF_END)&&(flash_addr_index+1 < msg.spi_flash_msg.num))
                        {
                            ...
                            if(GET_PLAY_DATA_BUFF_SIZE-flash_read_size < 44)
                            {
                                memset((char *)(data_buf+buf_index*GET_PLAY_DATA_BUFF_SIZE+flash_read_size),0x0,GET_PLAY_DATA_BUFF_SIZE-flash_read_size);
                                msg_data_size = GET_PLAY_DATA_BUFF_SIZE;
                                get_data_state = GET_PLAY_DATA_STATE_BUSY;
                                msg_data_flag = AUDIO_DATD_BUFF_NORMAL;
                                data_size = 0;
                        ...
                        do{
                            ...
                            if(offset_size != read_size)
                            {
                                ...
                                if(AUDIO_PLAY_OS_SUCCESS == audio_play_semaphore_take(download_end_sem,0))
                                {
                                    ...
                                    {
                                        /* 文件结束最后一笔数据 */
                                        memset((void *)(data_buf+buf_index*GET_PLAY_DATA_BUFF_SIZE+offset_size),0x00,read_size-offset_size);
                                        get_data_state = GET_PLAY_DATA_STATE_STOP;
                                        data_size = read_size;
           ...
           get_data_stop:
                /* clean */
                memset(data_buf,0,sizeof(uint8_t)*GET_PLAY_DATA_BUFF_SIZE*GET_PLAY_DATA_BUFF_COUNT);
                audio_play_queue_reset(data_buf_queue);
                #if AUDIO_PLAY_USE_SD_CARD
                f_close(&fsrc);
                #endif
...
int32_t read_weakup_voice_to_ram(uint32_t wakeup_voice_flash_addr)
{
    if(wakeup_voice_flash_addr != 0)
    {
        ...
        memset(wakeup_voice_buf,0x0,wave_head_size);
        ...
        else
        {
            switch(wakeup_voice_config.mode)
            {
                ...
                case CI_ADPCM_DECODER_MODE_FLAC:
                {
                    memset(data_buf+wave_total_lens+wave_head_size,0x0,4096-wave_total_lens-wave_head_size);
                    break;
                }
            }
        }
