...
{
    MP3ClearLastDecodeMainData(mp3_dec_handle);
    return 0;
}
...
{
    MP3SaveLastDecodeMainData(mp3_dec_handle,save_mp3_dec_handle);
    return 0;
}
...
{
    MP3LoadLastDecodeMainData(mp3_dec_handle,save_mp3_dec_handle);
    return 0;
}
#endif
...
int32_t audio_play_mp3_decode(uint8_t * input, uint32_t in_size, short *output,uint32_t *out_size, uint32_t *no_use_size,audio_format_info_t *audio_format_info)
{
    ...
    /* 寻找帧头 */
    mp3_sync_offset=MP3FindSyncWord(readPtr, bytesLeft);  
    ...
    else
    {
        ...
        /* 解码 */
        err = MP3Decode(mp3_dec_handle,&readPtr,(int *)(&bytesLeft),output,0);
        /* 取的帧信息 */
        MP3GetLastFrameInfo(mp3_dec_handle, &mp3FrameInfo);             
        if(ERR_MP3_NONE != err )
        {   
        ...
        else
        {
            if(fc->channels == 1)
            {
                *out_size = fc->max_blocksize*2;
                memset(output+fc->blocksize,0x0,(fc->max_blocksize-fc->blocksize)*2);
            }
            else
            {
                *out_size = fc->max_blocksize*4;
                memset(output+fc->blocksize*2,0x0,(fc->max_blocksize-fc->blocksize)*4);
            }
...
int32_t audio_play_ms_wav_decode(uint8_t * input, uint32_t in_size, short *output,uint32_t *out_size, uint32_t *no_use_size,audio_format_info_t *audio_format_info)
{
    ...
    memcpy(output,input,relly_size);
    #if !AUDIO_PLAYER_CONFIG_AUTO_PARSE_AUDIO_FILE
    if(strncmp((char *)input,"RIFF",4) == 0)
    {
        if(strncmp((char *)(input+0x24),"data",4) == 0)
        {
            memset(output,0x00,44);
        }
    }
    #endif
    *out_size = relly_size;
    *no_use_size = 0;
    if(*out_size < in_size)
    {
        memset(output+(*out_size)/sizeof(short),0x0,MS_WAV_IN_SIZE-(*out_size));
        *out_size = in_size;
    }
...
int32_t audio_play_adpcm_decode(uint8_t * input, uint32_t in_size, short *output,uint32_t *out_size, uint32_t *no_use_size,audio_format_info_t *audio_format_info)
{
    ...
    *no_use_size = 0;
    ...
    {
        memset(output+(*out_size)/sizeof(short),0x0,ADPCM_IN_SIZE*4-(*out_size));
        *out_size = in_size*4;
    }
...
int32_t audio_play_adpcm_decode2(uint8_t * input, uint32_t in_size, short *output,uint32_t *out_size, uint32_t *no_use_size,audio_format_info_t *audio_format_info)
{
    ...
    audio_format_info->nChans = 1;
    /* 这里*8的原因是ci112x系列iisdma传输是burst 16，而adpcm输出size一定是奇数*2 */
    ...
    if(block_num != temp_num)
    {
        ...
        if(relly_size > 0)
        {
            *output = *(short *)input;
            ...
            memset((void *)(output+2*(relly_size-4)+1),0x0,4*(adpcmBlockAlign-relly_size)+2*(2*(adpcmBlockAlign-4)+1)*(temp_num-block_num-1));
        }
        else
        {
            memset((void *)(output),0x0,2*(2*(adpcmBlockAlign-4)+1)*(temp_num-block_num));
        }
...
int32_t audio_play_aac_config( void *mode)
{
    ...
    {
        memcpy(&save_m4a_file_info,m4a_file_info,sizeof(m4a_file_info_t));
