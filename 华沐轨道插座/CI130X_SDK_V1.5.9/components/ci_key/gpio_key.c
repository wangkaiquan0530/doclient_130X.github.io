...
static void ci_key_gpio_isr_handle(void);

static gpio_irq_callback_list_t key_irq_callback = {ci_key_gpio_isr_handle,NULL};
...
void ci_key_gpio_hw_init(void)
{
    ...
    {
        scu_set_device_gate((uint32_t)gpio_key_list[i].gpio_group,ENABLE);
        #if GPIO_SET_IO_PULL_UP
        dpmu_set_io_pull(gpio_key_list[i].pinpad_name, DPMU_IO_PULL_UP);
        #endif
        //配置管脚使用GPIO功能
        dpmu_set_io_reuse(gpio_key_list[i].pinpad_name,FIRST_FUNCTION); //TODO:需要注意选用的按键引脚是否第一功能为GPIO功能
       ...
       // if(1 == i)//过零检测
        //{
       //     gpio_irq_trigger_config(gpio_key_list[i].gpio_group,gpio_key_list[i].pin_num,low_level_trigger);
       // }
       // else
        {
           //wkq  gpio_irq_trigger_config(gpio_key_list[i].gpio_group,gpio_key_list[i].pin_num,down_edges_trigger);
            gpio_irq_trigger_config(gpio_key_list[i].gpio_group,gpio_key_list[i].pin_num,low_level_trigger);
        }
...
#endif
        //使能中断
        if(gpio_key_list[i].gpio_group == PA)
        {
        	eclic_irq_enable(PA_IRQn);
        }
        else if(gpio_key_list[i].gpio_group == PB)
        {
...
void ci_key_gpio_init(void)
{
    ...
    ci_key_gpio_hw_init();
    registe_gpio_callback(PA, &key_irq_callback);
